...
public:
  ...
  Blob(It b, It e)
      : data(new std::vector<T>(b, e)) {} // ex 16.29, use new directly

private:
  SharedPtr<std::vector<T>> data; // throws msg if data[i] isn’t valid
...
template <typename T>
Blob<T>::Blob() : data(new std::vector<T>()) {} // ex 16.29, use new directly

template <typename T>
Blob<T>::Blob(std::initializer_list<T> il)
    : data(new std::vector<T>(il)) {} // ex 16.29, use new directly
...
template <typename T>
SharedPtr<std::vector<T>> BlobPtr<T>::check(std::size_t i,
