int main() {
  ...
  d = 42; // d is pointer to int, cannot assign integer literal to it; you can
          // write *d = 42 to change the object d points to
  e = 42; // d is pointer to const int, cannot assign integer literal to it
  g = 42; // g is a reference to const int, cannot assign new value to it
}